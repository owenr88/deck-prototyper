...
import { without, uniq, find } from 'lodash';
 ...
 * @param results 
 */
const validateFields = (results: Papa.ParseResult<PapaParseDataType>): Papa.ParseResult<PapaParseDataType> => {
  ...
  if( missingHeaders.length ) {
    throw new Error(`Missing the following fields: ${missingHeaders.join(', ')}`)
  }
 ...
 * @param cards 
 */
const createDecksFromImportedCards = (cards: CardTypeWithStringDecks[]): DeckType[] => {
  const deckNames: string[] = cards
    .reduce((all: string[], card: CardTypeWithStringDecks) => {
  ...
  const deckData: DeckType[] = deckNames
    .map((title, number) => ({
      number,
      title,
      description: '',
 ...
 * @param cards 
 */
const saveDecksAndCards = (decks: DeckType[], cards: CardTypeWithStringDecks[]) => {
